		...
		<form method="POST" action="" id="main_form">
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						<th><label><?php _e('赛博朋克翻译动效', 'argon');?></label></th>
						<td>
							<select name="argon_enable_cyberpunk_banner_effect">
								<?php $argon_enable_cyberpunk_banner_effect = get_option('argon_enable_cyberpunk_banner_effect', 'false'); ?>
								<option value="false" <?php if ($argon_enable_cyberpunk_banner_effect=='false'){echo 'selected';} ?>><?php _e('关闭', 'argon');?></option>
								<option value="true" <?php if ($argon_enable_cyberpunk_banner_effect=='true'){echo 'selected';} ?>><?php _e('开启', 'argon');?></option>
							</select>
							<p class="description"><?php _e('开启后将展示赛博朋克 2077 翻译动效，并自动禁用原打字机动画。', 'argon');?></p>
						</td>
					</tr>
					<tr>
						<th><label><?php _e('赛博朋克英文主标题', 'argon');?></label></th>
						<td>
							<input type="text" class="regular-text" name="argon_banner_title_en" value="<?php echo get_option('argon_banner_title_en'); ?>"/>
							<p class="description"><?php _e('留空则使用默认文案：WELCOME TO MY WORLD', 'argon');?></p>
						</td>
					</tr>
					<tr>
						<th><label><?php _e('赛博朋克英文副标题', 'argon');?></label></th>
						<td>
							<input type="text" class="regular-text" name="argon_banner_subtitle_en" value="<?php echo get_option('argon_banner_subtitle_en'); ?>"/>
							<p class="description"><?php _e('留空则使用默认文案：ENTER THE CYBER REALM', 'argon');?></p>
						</td>
					</tr>
					<tr><th class="subtitle"><h3><?php _e('外观', 'argon');?></h3></th></tr>
					<tr>
					...
					<tr>
						<th><label><?php _e('背景出处', 'argon');?></label></th>
						<td>
							<input type="text" class="regular-text" name="argon_page_background_source" value="<?php echo get_option('argon_page_background_source'); ?>"/>
							<p class="description"><?php _e('可填写背景图片的来源/作者/链接等信息。留空则不显示提示。', 'argon');?></p>
						</td>
					</tr>
					<tr>
						<th><label><?php _e('背景出处显示范围', 'argon');?></label></th>
						<td>
							<select name="argon_page_background_source_scope">
								<?php $argon_page_background_source_scope = get_option('argon_page_background_source_scope', 'home'); ?>
								<option value="all" <?php if ($argon_page_background_source_scope=='all'){echo 'selected';} ?>><?php _e('全页面生效', 'argon');?></option>
								<option value="home" <?php if ($argon_page_background_source_scope=='home'){echo 'selected';} ?>><?php _e('仅在主页生效', 'argon');?></option>
							</select>
							<p class="description"><?php _e('选择提示显示范围。仅在主页生效时，仅首页首屏加载显示；不响应 PJAX 跳转。', 'argon');?></p>
						</td>
					</tr>
...
window.pjaxLoaded = function(){
	...
	//do something...
                    ...
                    <tr><th class="subtitle" colspan="2"><h2><?php _e('以下为Mimosa新增功能配区~', 'argon');?></h2></th></tr>
                    <tr><th class="subtitle" colspan="2"><h3><?php _e('讨论区朋友标识', 'argon');?></h3></th></tr>
                    <tr>
                        <th scope="row">
                        ...
                        <td>
                            ...
                            <p class="description"><?php _e('每行输入一个需要显示「朋友」标识的邮箱，在发评论时对应的邮箱会被打上标识的', 'argon');?></p>
							 <!-- 示例图片 -->
							<div style="margin-top: 10px;">
								<strong style="display:block; margin-bottom: 4px;">示例：</strong>
								<img 
									src="<?php echo get_template_directory_uri(); ?>/assets/img/friend_icon.png" 
									alt="朋友标识示例" 
									style="height: 240px; vertical-align: middle;"
								/>
								<p>如果需更改标签内容，可参见 <a href="https://loneapex.cn/webcodeforbeauty#header-id-15" target="_blank">这篇文章</a> 中的相关代码。</p>
							</div>
                        </td>
                    </tr>
                    
                    <th class="subtitle" colspan="2"><h3><?php _e('背景遮罩不透明度', 'argon');?></h3></th>
                    <tr>
                        <th scope="row">
                            <label><?php _e('填入0~1之间的浮点数', 'argon');?></label>
                        </th>
                        <td>
							<input type="number" name="background_hide_degree" min="0" max="1000" value="<?php echo get_option('background_hide_degree', 175); ?>"/>
                            
                            <p class="description">'0~1之间，1为不透明，数字越小透明度越高。<br>这层遮罩是覆盖在原有壁纸（即argon_banner_background，包括日间、夜间）上的，如果不用填0即可</p>
                        </td>
                    </tr>


					<!-- 自定义统计的文章类型 -->
					<th class="subtitle" colspan="2"><h3><?php _e('文章字数统计', 'argon');?></h3></th>
					<tr>
					<th><label><?php _e('参与字数统计的文章类型', 'argon'); ?></label></th>
					<td>
						<input type="text" name="argon_mimosa_custom_post_types" value="<?php echo esc_attr(get_option('argon_mimosa_custom_post_types', 'post,shuoshuo')); ?>" />
						<p class="description"><?php _e('以英文逗号分隔，例如 post（长文章）,shuoshuo（argon说说文章）,page（页面）', 'argon'); ?></p>
					</td>
					</tr>

					<!-- 书籍映射 JSON -->
					<tr>
					<th><label><?php _e('字数对应的经典书籍映射（JSON 格式）', 'argon'); ?></label></th>
					<td>
						<textarea name="argon_mimosa_book_map" rows="8" cols="60"><?php
						echo esc_textarea(get_option('argon_mimosa_book_map', json_encode([
							50000   => '《变形记》',
							58000   => '《老人与海》',
							70000   => '《动物农庄》',
							97000   => '《鲁滨逊漂流记》',
						...
						<p class="description"><?php _e('使用 JSON 格式，键为字数，值为书名', 'argon'); ?></p>
						<p><b>输出样例（返回在页脚区域）：本站原创文章的总字数为 125,424 字（已刨除代码），已经接近 《月亮与六便士》（毛姆） 的篇幅了！</b></p>
						<p>若需要统计代码，可前往functions.php寻找「// 字数统计」注释，将「// 过滤代码块和标签」部分删去即可。</p>
						<p>如果需更改其内容，可参见 <a href="https://loneapex.cn/webcodeforbeauty#header-id-2" target="_blank">这篇文章</a> 中的相关代码。</p>

					</td>
					</tr>

					<!-- 弹窗设置 -->
					<th class="subtitle" colspan="2"><h3><?php _e('弹窗设置', 'argon');?></h3></th>

					<!-- 弹窗内容（HTML） -->
					<tr>
					<th><label for="argon_mimosa_popup_html"><?php _e('弹窗内容（HTML）', 'argon');?></label></th>
					<td>
						<textarea
						id="argon_mimosa_popup_html"
						name="argon_mimosa_popup_html"
						rows="8"
						cols="60"
						><?php echo esc_textarea( get_option('argon_mimosa_popup_html',
						'<div id="modal" class="popup_modal">
					<h2>通知</h2>
					<p class="tanchuang1">本站将于今晚（2025/7/17~2025/7/18）进行服务器迁移，部分服务可能出现阶段性不可用。</p>
					...
					/* your JS here */
						...
						<p class="description"><?php _e('填写整个弹窗的 HTML 结构，默认已包含关闭按钮。请保留默认的最外层div ', 'argon');?></p>
					</td>
					</tr>

					<!-- 弹窗触发频率 -->
					<tr>
					<th><label for="argon_mimosa_popup_frequency"><?php _e('弹窗触发频率', 'argon');?></label></th>
					<td>
						<select id="argon_mimosa_popup_frequency" name="argon_mimosa_popup_frequency">
						<?php $freq = get_option('argon_mimosa_popup_frequency', 'once'); ?>
						<option value="once"  <?php selected($freq,'once');  ?>><?php _e('仅显示一次', 'argon');?></option>
						<option value="daily" <?php selected($freq,'daily'); ?>><?php _e('每天显示', 'argon');?></option>
						<option value="custom"<?php selected($freq,'custom');?>><?php _e('自定义天数', 'argon');?></option>
						</select>
						<p class="description"><?php _e('选择弹窗在访客端的出现频率。', 'argon');?></p>
					</td>
					</tr>

					<!-- 自定义天数（仅当“自定义天数”时生效） -->
					<tr>
					<th><label for="argon_mimosa_popup_custom_days"><?php _e('自定义间隔天数', 'argon');?></label></th>
					<td>
						<input
						type="number"
						id="argon_mimosa_popup_custom_days"
						name="argon_mimosa_popup_custom_days"
						min="1"
						max="365"
						value="<?php echo esc_attr( get_option('argon_mimosa_popup_custom_days', 7) ); ?>"
						/>
						<p class="description"><?php _e('当“自定义天数”时，访客每隔多少天再次弹出。', 'argon');?></p>
					</td>
					</tr>
					

					<!-- 挂灯笼设置 -->
					<th class="subtitle" colspan="2"><h3><?php _e('挂赛博灯笼设置', 'argon');?></h3></th>

					<!-- 是否启用灯笼 -->
					<tr>
					<th><label for="argon_mimosa_lantern_enable"><?php _e('启用灯笼', 'argon');?></label></th>
					<td>
						<select id="argon_mimosa_lantern_enable" name="argon_mimosa_lantern_enable">
						<?php $enable = get_option('argon_mimosa_lantern_enable', 'no'); ?>
						<option value="yes" <?php selected($enable,'yes'); ?>><?php _e('启用', 'argon');?></option>
						<option value="no"  <?php selected($enable,'no');  ?>><?php _e('不启用', 'argon');?></option>
						</select>
						<p class="description"><?php _e('是否在页面上显示挂灯笼效果。', 'argon');?></p>
					</td>
					</tr>

					<!-- 生效设备 -->
					<tr>
					<th><label for="argon_mimosa_lantern_device"><?php _e('生效设备', 'argon');?></label></th>
					<td>
						<select id="argon_mimosa_lantern_device" name="argon_mimosa_lantern_device">
						<?php $device = get_option('argon_mimosa_lantern_device', 'desktop'); ?>
						<option value="desktop" <?php selected($device,'desktop'); ?>><?php _e('仅桌面端', 'argon');?></option>
						<option value="mobile"  <?php selected($device,'mobile');  ?>><?php _e('仅移动端（影响视线）', 'argon');?></option>
						<option value="both"    <?php selected($device,'both');    ?>><?php _e('桌面 + 移动端', 'argon');?></option>
						</select>
						<p class="description"><?php _e('选择灯笼在哪些设备上显示。', 'argon');?></p>
					</td>
					</tr>

					<!-- 灯笼文字自定义 -->
					<tr>
					<th><label><?php _e('灯笼文字（四个）', 'argon');?></label></th>
					<td>
						<input type="text" name="argon_mimosa_lantern_char1" value="<?php echo esc_attr(get_option('argon_mimosa_lantern_char1','乐')); ?>" size="4" maxlength="1" />
						<input type="text" name="argon_mimosa_lantern_char2" value="<?php echo esc_attr(get_option('argon_mimosa_lantern_char2','快')); ?>" size="4" maxlength="1" />
						<input type="text" name="argon_mimosa_lantern_char3" value="<?php echo esc_attr(get_option('argon_mimosa_lantern_char3','节')); ?>" size="4" maxlength="1" />
						<input type="text" name="argon_mimosa_lantern_char4" value="<?php echo esc_attr(get_option('argon_mimosa_lantern_char4','春')); ?>" size="4" maxlength="1" />
						<p class="description"><?php _e('分别对应右上、右下、左上、左下的灯笼文字，单字输入。可参考下图：', 'argon');?></p>
						<img 
									src="<?php echo get_template_directory_uri(); ?>/assets/img/lentern.jpg" 
									alt="示例" 
									style="height: 200px; vertical-align: middle;"
								/>
								<p>如需了解更多，可参见 <a href="https://loneapex.cn/archives/2699" target="_blank">这篇文章</a> <br>如需更改字体，建议直接引用博文中的源代码更改。在php中会有许多bug的……</p>
					</td>
					</tr>

					<!-- 悼念色设置 -->
					<th class="subtitle" colspan="2"><h3><?php _e('悼念色（灰度）', 'argon');?></h3></th>

					<tr>
					<th><label for="argon_mimosa_mourning_enable"><?php _e('启用悼念色（全站灰度）', 'argon');?></label></th>
					<td>
						<select id="argon_mimosa_mourning_enable" name="argon_mimosa_mourning_enable">
						<?php $m = get_option('argon_mimosa_mourning_enable','no'); ?>
						<option value="yes" <?php selected($m,'yes'); ?>><?php _e('启用', 'argon');?></option>
						<option value="no"  <?php selected($m,'no');  ?>><?php _e('关闭', 'argon');?></option>
						</select>
						<p class="description"><?php _e('开启后全站将应用灰度滤镜，用于哀悼或纪念。', 'argon');?></p>
						<p class="description"><?php _e('注：本功能可能存在bug，慎用', 'argon');?></p>
					</td>
					</tr>
					...
					<!-- RSS 设置 -->
					<th class="subtitle" colspan="2"><h3><?php _e('RSS Feed 设置', 'argon');?></h3></th>

					<!-- 是否把 “shuoshuo” 类型加入 RSS -->
					<tr>
					<th><label for="argon_mimosa_rss_include_shuoshuo"><?php _e('包含说说（shuoshuo）', 'argon');?></label></th>
					<td>
						<?php $inc = get_option('argon_mimosa_rss_include_shuoshuo','yes'); ?>
						<select id="argon_mimosa_rss_include_shuoshuo" name="argon_mimosa_rss_include_shuoshuo">
						<option value="yes" <?php selected($inc,'yes'); ?>><?php _e('是', 'argon');?></option>
						<option value="no"  <?php selected($inc,'no');  ?>><?php _e('否', 'argon');?></option>
						</select>
						<p class="description"><?php _e('是否在 RSS 中同时输出 “post” 和 “shuoshuo” 类型文章。', 'argon');?></p>
					</td>
					</tr>

					<!-- 密码保护文章在 RSS 中的处理 -->
					<tr>
					<th><label for="argon_mimosa_rss_password_behavior"><?php _e('密码保护文章', 'argon');?></label></th>
					<td>
						<?php $behave = get_option('argon_mimosa_rss_password_behavior','exclude'); ?>
						<select id="argon_mimosa_rss_password_behavior" name="argon_mimosa_rss_password_behavior">
						<option value="exclude" <?php selected($behave,'exclude'); ?>><?php _e('从 RSS 排除', 'argon');?></option>
						<option value="prompt"  <?php selected($behave,'prompt');  ?>><?php _e('只显示密码输入提示', 'argon');?></option>
						</select>
						<p class="description"><?php _e('“排除”会完全去掉；“只显示密码输入提示”会在 RSS 中输出密码表单而不显示正文。', 'argon');?></p>
					</td>
					</tr>
...
function argon_update_themeoptions(){
	...
	if ($_POST['update_themeoptions'] == 'true'){
		...
		argon_update_option_checkbox('argon_enable_cyberpunk_banner_effect');
		argon_update_option('argon_banner_title_en');
		argon_update_option('argon_banner_subtitle_en');
		...
		argon_update_option('argon_page_background_source');
		argon_update_option('argon_page_background_source_scope');
		...
		argon_update_option_allow_tags('background_hide_degree');

		//Mimosa新增功能
		argon_update_option_allow_tags('argon_mimosa_custom_post_types');
		argon_update_option_allow_tags('argon_mimosa_book_map');
		argon_update_option_allow_tags('argon_mimosa_popup_html');
		argon_update_option('argon_mimosa_popup_frequency');
		argon_update_option('argon_mimosa_popup_custom_days');
		argon_update_option('argon_mimosa_lantern_enable');
		argon_update_option('argon_mimosa_lantern_device');
		argon_update_option('argon_mimosa_lantern_char1');
		argon_update_option('argon_mimosa_lantern_char2');
		argon_update_option('argon_mimosa_lantern_char3');	
		argon_update_option('argon_mimosa_lantern_char4');
		argon_update_option('argon_mimosa_mourning_enable');
		argon_update_option('argon_mimosa_rss_include_shuoshuo');
		argon_update_option('argon_mimosa_rss_password_behavior');
