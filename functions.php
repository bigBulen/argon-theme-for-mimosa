<?php
// 引入时间轴日历系统
require_once get_template_directory() . '/includes/timeline-calendar.php';
// 站点提示（GitHub Issue 风格）
require_once get_template_directory() . '/includes/site-notices.php';
// 追番 / galgame 功能
require_once get_template_directory() . '/includes/apex-media-list.php';

function argon_get_home_feed_combined($paged = null) {
    global $apex_media_list;

    $paged = $paged ? intval($paged) : 0;
    if ($paged <= 0) {
    ...
    if ($apex_media_list instanceof Apex_Media_List) {
        $media_total = $apex_media_list->media_query_count([
...
function argon_render_home_feed_pagination($total, $per_page, $paged) {
    ...
    return get_argon_formatted_paginate_links_for_all_platforms();
}



// 生日 变量声明
function apex_set_birthday_theme_vars() {
    global $custom_bg_url, $custom_bg_dark_url, $banner_subtitle, $bg_overlay_alpha;
    global $todayData, $birthday_name_cn, $birthday_name_jp, $birthday_quote, $birthday_description, $suffix;
	global $tittle;

    $hide_de = get_option('background_hide_degree');
    // —— 读取 birth.json ——
	// 若使用远程文件（正式环境使用相对链接，远程文件供调试使用）：
	// $json_url = 'https://loneapex.cn/character_collections/birth.json';
    // $response = wp_remote_get( $json_url );
    // $births   = [];

    $birth_file = get_stylesheet_directory() . '/character_collections/birth.json';
	$births = [];
	if ( file_exists( $birth_file ) ) {
		$json = file_get_contents( $birth_file );
		$births = json_decode( $json, true );
	}

    $today = date( 'm-d', current_time( 'timestamp' ) );
    // error_log( "[Birthday Debug] today={$today}, births=" . print_r( $births, true ) );

    // —— 匹配今日生日 ——
    $filtered = array_filter( $births, function( $item ) use ( $today ) {
        return ! empty( $item['birthday'] ) && $item['birthday'] === $today;
    } );
    // 重建索引，保证数值索引
    ...
    if ( ! empty( $matches ) ) {
        // 随机选一位
        $todayData = $matches[ array_rand( $matches ) ];
    } else {
        $todayData = null;
    }
    ...
    // —— 背景 URL 和遮罩透明度 ——
    if ( $todayData ) {
		$img_base_based = get_template_directory_uri();
        $img_base = $img_base_based. '/character_collections/img/';
        $custom_bg_url      = $img_base . $todayData['id'] . '.jpg';
        $custom_bg_dark_url = $img_base . $todayData['id'] . '.jpg';
        $bg_overlay_alpha   = isset( $todayData['alpha'] ) ? floatval( $todayData['alpha'] ) : 0.1;

        
        $birthday_name_jp     = $todayData['name_jp'];
        $birthday_quote       = $todayData['quote'];
        $birthday_description = $todayData['description'];
		$tittle               = $todayData['tittle'];

        $banner_subtitle = '重要的日子 | '. $tittle;
    } else {
        $custom_bg_url      = apply_filters( 'argon_page_background_url', get_option( 'argon_page_background_url' ) );
        $custom_bg_dark_url = apply_filters( 'argon_page_background_dark_url', get_option( 'argon_page_background_dark_url' ) );
        $bg_overlay_alpha   = $hide_de;
...
add_action( 'wp', 'apex_set_birthday_theme_vars' );



// rss系列设置
// 1) RSS 查询：是否包含 shuoshuo，是否排除密码文
add_filter('pre_get_posts', 'argon_mimosa_rss_query');
function argon_mimosa_rss_query($query) {
    if ( ! $query->is_main_query() || ! $query->is_feed() ) {
        return $query;
    }

    // 包含 shuoshuo?
    if ( get_option('argon_mimosa_rss_include_shuoshuo','yes') === 'yes' ) {
        $query->set('post_type', ['post','shuoshuo']);
    }

    // 密码保护文章行为
    $behavior = get_option('argon_mimosa_rss_password_behavior','exclude');
    if ( $behavior === 'exclude' ) {
...
// 2) 密码保护 → 仅输出密码表单
add_filter('the_content_feed', 'argon_mimosa_rss_protected_prompt', 10, 1);
function argon_mimosa_rss_protected_prompt($content) {
    if ( ! is_feed() ) {
        return $content;
    }
    // 仅在 “只显示密码输入提示” 模式下处理
    if ( get_option('argon_mimosa_rss_password_behavior','exclude') !== 'prompt' ) {
        return $content;
    }
    global $post;
...
function add_default_title_to_rss_shuoshuo($title, $post_id) {
    if ( is_feed() && get_post_type($post_id) === 'shuoshuo' ) {
        return '碎碎念 # ' . $post_id . ' ~ ' . $title;
    }
    return $title;
}
...
// 密码提示
// 1. 注册 Meta Box
function add_password_hint_meta_box() {
    add_meta_box(
        'password_hint_meta',           // 唯一 ID
        '密码提示',                     // 标题
        'render_password_hint_meta_box',// 回调函数
        'post',                         // 用于文章 post 类型
        'side',                         // 显示在侧边
        'default'                       // 默认优先级
    );
}
add_action( 'add_meta_boxes', 'add_password_hint_meta_box' );

// 2. 渲染 Meta Box 内容
function render_password_hint_meta_box( $post ) {
    $value = get_post_meta( $post->ID, 'password_hint', true );
    wp_nonce_field( 'save_password_hint', 'password_hint_nonce' );
    echo '<textarea name="password_hint" style="width:100%;min-height:60px;">'
         . esc_textarea( $value )
         . '</textarea>';
    echo '<p class="description">访客看到的密码提示，不会自动加上“密码提示：”前缀。</p>';
}

// 3. 保存 Meta Box 数据
function save_password_hint_meta_box( $post_id ) {
    // 校验 nonce
    if ( ! isset( $_POST['password_hint_nonce'] ) ||
         ! wp_verify_nonce( $_POST['password_hint_nonce'], 'save_password_hint' ) ) {
        return;
    }
    // 自动保存、修订等不处理
    ...
    if ( isset( $_POST['password_hint'] ) ) {
        update_post_meta(
            $post_id,
            'password_hint',
            sanitize_text_field( wp_unslash( $_POST['password_hint'] ) )
        );
    }
}
add_action( 'save_post', 'save_password_hint_meta_box' );



/**
 * 将 get_avatar() 输出里的 cravatar.cn/avatar 换成 cravatar.com/avatar
 *
 * @param string $avatar_html Avatar 的 HTML 片段
 * @return string 替换域名后的 HTML
 */
function replace_cravatar_domain_in_avatar_html( $avatar_html ) {
    return str_replace(
...
add_filter( 'get_avatar', 'replace_cravatar_domain_in_avatar_html', 10, 1 );

/**
 * 将 get_avatar_url() 返回的 URL 里的 cravatar.cn/avatar 换成 cravatar.com/avatar
 *
 * @param string $url Avatar 的 URL
 * @return string 替换域名后的 URL
 */
function replace_cravatar_domain_in_avatar_url( $url ) {
    return str_replace(
        'https://cravatar.cn/avatar',
        'https://cravatar.com/avatar',
        $url
...
add_filter( 'get_avatar_url', 'replace_cravatar_domain_in_avatar_url', 10, 1 );



// 实况图片
function enqueue_livephotoskit_script() {
    wp_enqueue_script('livephotoskit-js', 'https://loneapex.cn/extra-js/livephotoskit.js', array(), null, true);
}
add_action('wp_enqueue_scripts', 'enqueue_livephotoskit_script');

function enqueue_custom_livephotos_init_script() {
    wp_enqueue_script(
        'custom-livephotos-init',
        get_template_directory_uri() . '/custom-live-init.js',
        array(),
        filemtime(get_template_directory() . '/custom-live-init.js'),
        true
    );
}
add_action('wp_enqueue_scripts', 'enqueue_custom_livephotos_init_script');
...
function render_custom_live_photos_block($attributes) {
    ...
    $width = '100%';
    $height = '600px'; // Example fixed height, adjust as necessary
...
// 赛博灯笼
add_action( 'wp_footer', 'argon_mimosa_render_lanterns' );
function argon_mimosa_render_lanterns() {
    // 读取设置
    if ( get_option( 'argon_mimosa_lantern_enable', 'no' ) !== 'yes' ) {
        return;
    }
    $device = get_option( 'argon_mimosa_lantern_device', 'desktop' );
    $chars = [
        get_option( 'argon_mimosa_lantern_char1', '乐' ),
        get_option( 'argon_mimosa_lantern_char2', '快' ),
        get_option( 'argon_mimosa_lantern_char3', '节' ),
        get_option( 'argon_mimosa_lantern_char4', '春' ),
    ];

    // 输出 CSS
    echo '<style>
    .deng-box, .deng-box1, .deng-box2, .deng-box3 { position: fixed; z-index: 9999; pointer-events: none; }
    .deng-box  { top: -40px; left:  55px; }
    .deng-box1 { top: -30px; left: -50px; }
    .deng-box2 { top: -30px; right: 55px; }
    .deng-box3 { top: -40px; right:-50px; }
    .deng, .deng-box1 .deng, .deng-box2 .deng {
      ...
      transform-origin:50% -100px; animation:swing 4s infinite ease-in-out;
      box-shadow:-5px 5px 50px 4px rgba(250,108,0,1);
    }
    .deng-a { width:100px; height:90px; margin:12px 8px 8px 10px;
      background: rgba(216,0,15,0.1); border:2px solid #dc8f03; border-radius:50% 50%; }
    .deng-b { width:45px; height:90px; margin:-2px 8px 8px 26px;
      background: rgba(26,0,15,0.1); border:2px solid #dc8f03; border-radius:50% 50%; }
    .xian { position:absolute; top:-20px; left:60px; width:2px; height:20px; background:#dc8f03; }
    .shui-a { position:relative; width:5px; height:20px; margin:-5px 0 0 59px;
      transform-origin:50% -45px; animation:swing 4s infinite ease-in-out;
      background:#ffa500; border-radius:0 0 5px 5px; }
    .shui-b { position:absolute; top:14px; left:-2px; width:10px; height:10px;
      background:#dc8f03; border-radius:50%; }
    .shui-c { position:absolute; top:18px; left:-2px; width:10px; height:35px;
      background:#ffa500; border-radius:0 0 0 5px; }
    .deng:before, .deng:after {
      content:" "; position:absolute; display:block; width:60px; height:12px;
      border:solid 1px #dc8f03; background:linear-gradient(to right,#dc8f03,#ffa500,#dc8f03,#ffa500,#dc8f03);
    }
    .deng:before { top:-7px; left:29px; border-radius:5px 5px 0 0; }
    .deng:after  { bottom:-7px; left:10px; border-radius:0 0 5px 5px; }
    .deng-t {
      font-family:lanterns,sans-serif; font-size:3.2rem; color:#dc8f03;
      font-weight:bold; line-height:85px; text-align:center;
      margin-left:-4.5px; margin-top:4px;
    }
    @keyframes swing {
    ...
    // MobileDetect 脚本
    echo '<script src="https://loneapex.cn/extra-js/beauty/mobile-detect.js"></script>';
    ...
    (function(){
      ...
      var device = '<?php echo esc_js( $device ); ?>';
      if ( (device === 'desktop' && isPhone)
        || (device === 'mobile'  && !isPhone ) ) {
        return;
      }

      var chars = <?php echo wp_json_encode( $chars ); ?>;
      var html = `
        <div class="deng-box3"><div class="deng"><div class="xian"></div>
          <div class="deng-a"><div class="deng-b"><div class="deng-t">${chars[0]}</div></div></div>
          <div class="shui shui-a"><div class="shui-c"></div><div class="shui-b"></div></div>
        </div></div>

        <div class="deng-box2"><div class="deng"><div class="xian"></div>
          <div class="deng-a"><div class="deng-b"><div class="deng-t">${chars[1]}</div></div></div>
          <div class="shui shui-a"><div class="shui-c"></div><div class="shui-b"></div></div>
        </div></div>

        <div class="deng-box"><div class="deng"><div class="xian"></div>
          <div class="deng-a"><div class="deng-b"><div class="deng-t">${chars[2]}</div></div></div>
          <div class="shui shui-a"><div class="shui-c"></div><div class="shui-b"></div></div>
        </div></div>

        <div class="deng-box1"><div class="deng"><div class="xian"></div>
          <div class="deng-a"><div class="deng-b"><div class="deng-t">${chars[3]}</div></div></div>
          <div class="shui shui-a"><div class="shui-c"></div><div class="shui-b"></div></div>
        </div></div>
      `;
...
add_action( 'admin_init', function(){
    register_setting( 'argon_settings_group', 'argon_mimosa_mourning_enable', 'sanitize_text_field' );
} );

add_action( 'wp_head', 'argon_mimosa_apply_mourning_color', 1 );
function argon_mimosa_apply_mourning_color() {
    if ( get_option( 'argon_mimosa_mourning_enable', 'no' ) !== 'yes' ) {
        return;
    }
    // 最简单的灰度测试，带 !important 保证覆盖
    echo '<style>html, body { filter: grayscale(100%) !important; }</style>';
}
			...
			<div class="comment-item-title">
				<div class="comment-name">
					...
					<?php if (user_can($comment -> user_id , "update_core")){
						echo '<span class="badge badge-primary badge-admin">' . __('站长', 'argon') . '</span>';}
					?>
...
function argon_comment_cmp($a, $b){
	...
	// 置顶优先
	if ($a_pinned === 'true' && $b_pinned !== 'true') return -1;
	if ($a_pinned !== 'true' && $b_pinned === 'true') return 1;
...
function argon_output_background_source_toast(){
	if (is_admin()) { return; }
	$source = trim((string) get_option('argon_page_background_source', ''));
	if ($source === '') { return; }

	$scope = get_option('argon_page_background_source_scope', 'home');
	if ($scope !== 'all') {
		if (!(is_front_page() || is_home())) { return; }
	}
	?>
	<style>
	.argon-bgsource-toast{position:fixed;top:60px;right:20px;z-index:9999;max-width:320px;background:rgba(0, 0, 0, 0.66);color:#fff;padding:10px 14px;border-radius:8px;box-shadow:0 4px 12px rgba(0,0,0,.2);font-size:14px;line-height:1.5;opacity:0;transform:translateY(-8px);transition:opacity .2s ease, transform .2s ease}
	.argon-bgsource-toast.show{opacity:1;transform:translateY(0)}
	.argon-bgsource-toast .progress{height:3px;background:rgba(26, 46, 0, 0.2);border-radius:2px;overflow:hidden;margin-top:8px}
	.argon-bgsource-toast .progress .bar{width:100%;height:100%;background:rgba(255,255,255,.7);animation:argon-bgsource-progress 3.5s linear forwards}
	@keyframes argon-bgsource-progress{from{width:100%}to{width:0}}
	.argon-bgsource-toast .close{position:absolute;top:6px;right:8px;cursor:pointer;color:#fff;opacity:.7;font-size:16px;line-height:1}
	.argon-bgsource-toast .close:hover{opacity:1}
	</style>
	<script>
	(function(){
		// 仅首屏加载显示，不绑定 PJAX 事件
		if (window.__argonBgSourceToastShown) return;
		window.__argonBgSourceToastShown = true;
		var text = <?php echo json_encode(esc_html($source)); ?>;
		var t = document.createElement('div');
		t.className = 'argon-bgsource-toast';
		t.setAttribute('role','status');
		t.setAttribute('aria-live','polite');
		t.innerHTML = '<span class="close" aria-label="close">×</span>' + text + '<div class="progress"><div class="bar"></div></div>';
		function remove(){ if(!t) return; t.classList.remove('show'); setTimeout(function(){ if(t&&t.parentNode){t.parentNode.removeChild(t);} t=null; },200); }
		var onReady=function(){
			document.body.appendChild(t);
			setTimeout(function(){ if(t) t.classList.add('show'); },20);
			setTimeout(remove,3500);
			t.querySelector('.close').addEventListener('click', remove);
		};
		if (document.readyState==='complete'||document.readyState==='interactive'){ setTimeout(onReady,0); }
		else { document.addEventListener('DOMContentLoaded', onReady); }
	})();
	</script>
	<?php
}
add_action('wp_footer', 'argon_output_background_source_toast');


//Gutenberg 编辑器 悬浮工具栏 增加 隐藏文本 格式
function argon_enqueue_hidden_text_format_for_block_editor() {
	$js_path = get_template_directory() . '/gutenberg/argon-hidden-text-format.js';
	if ( !file_exists($js_path) ){ return; }
	wp_enqueue_script(
		'argon-hidden-text-format',
		get_template_directory_uri() . '/gutenberg/argon-hidden-text-format.js',
		array('wp-rich-text', 'wp-element', 'wp-components', 'wp-block-editor', 'wp-editor'),
		filemtime($js_path),
		true
	);

	$css_path = get_template_directory() . '/gutenberg/argon-hidden-text-format.css';
	if ( file_exists($css_path) ) {
		wp_enqueue_style(
			'argon-hidden-text-format-style',
			get_template_directory_uri() . '/gutenberg/argon-hidden-text-format.css',
			array(),
			filemtime($css_path)
		);
	}
}
add_action('enqueue_block_editor_assets', 'argon_enqueue_hidden_text_format_for_block_editor');


//Gutenberg 编辑器 ACGN卡片
add_action('init', function () {
    register_block_type('apex/media-card', [
        'attributes'      => [
            'mediaId' => [
                'type'    => 'number',
                'default' => 0,
        ...
        'render_callback' => 'apex_render_media_card_block',
    ]);
});

function apex_render_media_card_block($attrs) {
    if (empty($attrs['mediaId'])) return '';

    global $apex_media_list;

    if (!($apex_media_list instanceof Apex_Media_List)) {
        return '<div>媒体组件未初始化</div>';
    }

    $media_item = $apex_media_list->get_media_row_by_id(intval($attrs['mediaId']));
    if (!$media_item) {
        return '<div>找不到对应条目</div>';
    }
    ...
    return '<div class="apex-media-card--in-post">' . ob_get_clean() . '</div>';
}

add_action('enqueue_block_editor_assets', function () {
    wp_enqueue_script(
        'apex-media-card-block',
        get_template_directory_uri() . '/assets/blocks/media-card.js',
